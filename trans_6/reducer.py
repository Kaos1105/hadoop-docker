...
# GameType -> { players: set(), amount: float, ids: list() }
...
for line in sys.stdin:
    ...
    if parts[0] == "PLAYER":
        first_name = parts[1]
        age = parts[2]
        cust_to_info[cust_id] = (first_name, age)
...
for game_type, data in game_summary.items():
    players = set()
    for cid in data["ids"]:
        if cid in cust_to_info:
            fname, age = cust_to_info[cid]
            players.add(f"{fname},{age}")
        else:
            players.add(cid)  # fallback
